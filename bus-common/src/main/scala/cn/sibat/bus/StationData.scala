...
//        var firstRow: Row = null
        //val maybeLineId = new ArrayBuffer[String]()
...
//          }
          gps = gps.map{row=>
            var result = row
            val spllit = row.split(",")
            val lon = spllit(8).toDouble
            val lat = spllit(9).toDouble
            ...
            if (array.size > 1) {
              ...
              if (min2.sum < 1.2 * pd) {
                result = result+","+ route._1//route.getString(route.fieldIndex("route"))
              }
            }
            result
          }
